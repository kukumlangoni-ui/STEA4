        ...
        {isDataUrl ? (
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            ...
            {value && (
              <button 
                onClick={() => onChange("")} 
                style={{ position: 'absolute', right: 10, background: "transparent", border: "none", color: "rgba(255,255,255,.5)", cursor: "pointer", fontSize: 16 }}
              >
                ✕
              </button>
            )}
      ...
      <div style={{ display:"grid", gap:12 }}>
        ...
        {docs.filter(d => (d.name||"").toLowerCase().includes(search.toLowerCase())).map(item=>(
          <div key={item.id} style={{ borderRadius:16, border:`1px solid ${item.active?"rgba(255,255,255,.07)":"rgba(239,68,68,.2)"}`, background:item.active?"#1a1d2e":"rgba(239,68,68,.05)", padding:"14px 18px", display:"flex", gap:12, alignItems:"center", flexWrap:"wrap", maxWidth: 800 }}>
            ...
            <div style={{ width: '100%' }}>
              <SocialPoster content={item} type="Deal" />
            </div>
          </div>
        ))}
      </div>
