...
export default function AdminPanel({ user, onBack }) {
  ...
  const [counts,  setCounts]  = useState({ tips:0, updates:0, deals:0, courses:0, users:0, products:0, websites:0, prompts:0, sponsored_ads:0, orders:0, subscriptions:0, payments:0, deliveries:0, message_templates:0 });

  const db = getFirebaseDb();

  useEffect(() => {
    if (!db) return;
    const cols = ["tips","updates","deals","courses","users","products","websites","prompts", "sponsored_ads", "orders", "subscriptions", "payments", "deliveries", "message_templates"];
