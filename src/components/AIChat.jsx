  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "12px 16px" }}>
      ...
      <style>{`
        ...
        @keyframes steaFadeUp { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)} }
        @keyframes steaFloat { 0%,100%{transform:translateY(0px)} 50%{transform:translateY(-6px)} }
...
function MsgBubble({ msg }) {
  ...
  return (
    <div className="stea-msg" style={{ display:"flex", justifyContent:isUser?"flex-end":"flex-start", marginBottom:14 }}>
      {!isUser && (
        <div style={{ width:30,height:30,borderRadius:10,flexShrink:0,marginRight:8,marginTop:2,
          background:`linear-gradient(135deg,${G},${G2})`,display:"grid",placeItems:"center" }}>
          <Zap size={14} color="#111" strokeWidth={2.5}/>
        </div>
      )}
      <div style={{
        maxWidth:"78%", padding: isUser?"10px 14px":"12px 16px",
        borderRadius: isUser?"18px 4px 18px 18px":"4px 18px 18px 18px",
        background: isUser?`linear-gradient(135deg,${G},${G2})`:"rgba(255,255,255,0.07)",
        border: isUser?"none":"1px solid rgba(255,255,255,0.08)",
        color: isUser?"#111":"#fff", fontSize:14, lineHeight:1.65, fontWeight: isUser?600:400,
        wordBreak:"break-word", backdropFilter: isUser?"none":"blur(10px)",
        boxShadow: isUser?"0 4px 20px rgba(245,166,35,0.25)":"0 2px 12px rgba(0,0,0,0.2)",
      }}>
...
VIBE / TONE DETECTION
TIER 1 — YOUNG / CASUAL: uses "Inakuaje","Oya","Niaje","Mambo","Vipi","Bro","Mkuu" → respond informally, energetic, use light slang naturally.
TIER 2 — FORMAL: uses "Shikamoo","Habari","Naomba msaada" → respond respectfully, clearly, calmly.
TIER 3 — FAMILIAR: returning user → respond warmly. If uncertain, use neutral friendly tone.

LANGUAGE RULES
- Kiswahili by default
- English only for tech terms: Update, Backup, Storage, Settings, Link, Software, Browser, App
- Explain tech terms simply for beginners
- Avoid robotic Kiswahili

ANSWER STYLE
Short to medium, easy to scan, practical, direct.
Structure: 1) Direct answer 2) Simple explanation 3) Clear steps 4) Warning/tip if needed.

AUTHENTICITY RULES
Never say "I am a large language model" or "As an AI..."
Answer naturally as STEA AI. If uncertain, say so simply and guide to next step.

HELPFULNESS RULE
Every answer must: solve the problem, give steps, suggest next action, or ask a useful follow-up. No vague answers.

STEA TOUCH
Lightly mention STEA when relevant: "Hapa STEA tuna guides za aina hiyo", "STEA tuko kwa tech kwa Kiswahili"
Don't force promotional mentions.

CLOSING STYLE
End naturally: "Ukiwa tayari, nakuelekeza hatua inayofuata." / "Ukikwama popote, niambie." / "Tupo pamoja."

STEA OUTRO — only at end of longer helpful answers:
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",height:"100%",width:"100%",
      ...
      <div style={{ position:"absolute",top:-60,right:-60,width:220,height:220,borderRadius:"50%",
        background:"radial-gradient(circle,rgba(245,166,35,0.12) 0%,transparent 70%)",pointerEvents:"none",zIndex:0 }}/>
      ...
      <div style={{ position:"relative",zIndex:10,padding:"14px 18px",
        ...
        <div style={{ display:"flex",alignItems:"center",gap:12 }}>
          {view === "chat" ? (
            <button onClick={()=>setView("home")} style={{ width:34,height:34,borderRadius:10,
              border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.05)",
              color:"rgba(255,255,255,0.6)",cursor:"pointer",display:"grid",placeItems:"center" }}>
              <ChevronLeft size={16}/>
            </button>
          ) : (
            <div style={{ width:40,height:40,borderRadius:13,background:`linear-gradient(135deg,${G},${G2})`,
              display:"grid",placeItems:"center",boxShadow:`0 0 20px rgba(245,166,35,0.3)`,
        ...
        <button onClick={onClose} style={{ width:32,height:32,borderRadius:"50%",
          ...
          color:"rgba(255,255,255,0.4)",cursor:"pointer",display:"grid",placeItems:"center" }}
        ...
        {view === "home" && (
          <div style={{ height:"100%",overflowY:"auto",padding:"28px 20px 20px" }} className="stea-chat-scroll">
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:10,marginBottom:24 }}>
              <button onClick={()=>setView("chat")} style={{ padding:"14px 18px",borderRadius:16,border:"none",
                background:`linear-gradient(135deg,${G},${G2})`,color:"#111",fontWeight:800,fontSize:14,cursor:"pointer",
                display:"flex",alignItems:"center",justifyContent:"space-between",
                boxShadow:`0 8px 24px rgba(245,166,35,0.3)`,transition:"transform 0.2s,box-shadow 0.2s" }}
              ...
              <a href="https://wa.me/255752661307" target="_blank" rel="noopener noreferrer" style={{
                padding:"14px 18px",borderRadius:16,border:"1px solid rgba(37,211,102,0.2)",
                background:"rgba(37,211,102,0.06)",color:"#fff",fontWeight:700,fontSize:14,
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
                {quickActions.map(a => (
                  <button key={a.label} onClick={()=>handleSend(a.prompt)} style={{
                    ...
                    color:"rgba(255,255,255,0.7)",fontSize:12.5,fontWeight:600,cursor:"pointer",lineHeight:1.5,transition:"all 0.2s" }}
          ...
          <div style={{ height:"100%",overflowY:"auto",padding:"20px 16px 8px" }} className="stea-chat-scroll">
            ...
            {loading && (
              <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:14 }}>
                <div style={{ width:30,height:30,borderRadius:10,flexShrink:0,background:`linear-gradient(135deg,${G},${G2})`,display:"grid",placeItems:"center" }}>
                  <Zap size={14} color="#111" strokeWidth={2.5}/>
                </div>
      ...
      <div style={{ position:"relative",zIndex:10,padding:"12px 16px 16px",
        borderTop:"1px solid rgba(255,255,255,0.06)",background:"rgba(0,0,0,0.3)",backdropFilter:"blur(20px)" }}>
        {view === "home" ? (
          <div style={{ display:"flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.04)",
            border:"1px solid rgba(255,255,255,0.08)",borderRadius:18,padding:"6px 8px 6px 16px" }}
            ...
            <button onClick={()=>handleSend()} disabled={!input.trim()} style={{
              ...
              cursor:input.trim()?"pointer":"default",display:"grid",placeItems:"center",transition:"all 0.2s" }}>
              <Send size={16}/>
            </button>
          </div>
        ) : (
          <div style={{ display:"flex",alignItems:"flex-end",gap:10,background:"rgba(255,255,255,0.04)",
            border:"1px solid rgba(255,255,255,0.08)",borderRadius:18,padding:"8px 8px 8px 16px" }}
            onFocusCapture={e=>e.currentTarget.style.borderColor="rgba(245,166,35,0.4)"}
            onBlurCapture={e=>e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}>
            <textarea ref={inputRef} value={input}
              onChange={e=>{setInput(e.target.value);e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,120)+"px";}}
              onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();handleSend();}}}
              placeholder="Andika swali lako... (Enter kutuma)" rows={1}
              style={{ flex:1,background:"transparent",border:"none",outline:"none",color:"#fff",fontSize:14,lineHeight:1.6,resize:"none",fontFamily:"inherit",maxHeight:120,overflowY:"auto" }}/>
            <button onClick={()=>handleSend()} disabled={loading||!input.trim()} style={{
              width:40,height:40,borderRadius:13,flexShrink:0,border:"none",
              ...
              cursor:(!loading&&input.trim())?"pointer":"default",display:"grid",placeItems:"center",transition:"all 0.2s",
