  ...
  return (
    <section style={{ padding: isMobile ? "12px 0 10px" : "22px 0 16px" }}>
      <W>
        ...
        >
          {/* StarCanvas and EarthHero removed for mobile performance */}

          {/* Texture overlay removed - external URL hurts mobile performance */}
  ...
  useEffect(() => {
    ...
    const unsubAds = onSnapshot(q, (snap) => {
      ...
      if (activePopup && !localStorage.getItem(`popup_shown_${activePopup.id}`)) {
        ...
        t = setTimeout(() => setShowPopup(true), 8000); // Show after 8s - let page render first
      }
    });
  ...
  return (
    <>
      ...
      {adminOpen ? (
        ...
        >
          <style>{`*{box-sizing:border-box;margin:0;padding:0}input::placeholder{color:rgba(255,255,255,.28)}textarea::placeholder{color:rgba(255,255,255,.28)}`}</style>
