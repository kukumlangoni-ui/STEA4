...
function AuthModal({ onClose, onUser }) {
  ...
  const switchTo = (m) => {
    if (m === "register") setTog(true);
    else if (m === "login") setTog(false);
    setMode(m);
    setErr("");
  };
  ...
  const doGoogle = async () => {
    const auth = getFirebaseAuth();
    if (!auth) { setErr("Firebase haijasanidiwa."); return; }
    ...
    try {
      ...
      if (msg.includes("auth/popup-blocked")) msg = "Popup imezuiwa. Ruhusu popups kwenye browser.";
      if (msg.includes("auth/unauthorized-domain")) msg = "Domain hii haijaruhusiwa kwenye Firebase.";
      setErr(msg);
  ...
  const doEmail = async () => {
    ...
    try {
      ...
      let msg = e.message.replace("Firebase:", "").trim();
      if (msg.includes("auth/user-not-found")) msg = "Akaunti hii haipo. Tafadhali jisajili.";
      if (msg.includes("auth/wrong-password")) msg = "Password si sahihi.";
      if (msg.includes("auth/invalid-credential")) msg = "Email au Password si sahihi.";
      if (msg.includes("auth/email-already-in-use")) msg = "Email hii tayari inatumika.";
      if (msg.includes("auth/invalid-email")) msg = "Email si sahihi.";
      setErr(msg);
    } finally { setLoading(false); }
  };
  ...
  const doForgot = async () => {
    ...
    try {
      ...
      setErr("✅ Reset link imetumwa kwa email yako!");
  ...
  const cardStyle = {
    ...
    boxShadow: "0 40px 120px rgba(0,0,0,0.8), 0 0 0 1px rgba(245,166,35,0.06)",
    display: "flex",
    overflow: "hidden",
  };
  ...
  return createPortal(
    <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={cardStyle}>
        ...
        {/* LEFT — Form panel */}
        <div style={{ flex: 1, padding: "48px 48px 40px", display: "flex", flexDirection: "column", justifyContent: "center", minWidth: 0 }}>

          {/* Tab switcher */}
          {mode !== "forgot" && (
            <div style={{ display: "inline-flex", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 99, padding: 5, marginBottom: 32, alignSelf: "flex-start" }}>
              {["login", "register"].map(m => (
                <button key={m} onClick={() => switchTo(m)} style={{
                  padding: "8px 24px", borderRadius: 99, border: "none", cursor: "pointer",
                  fontSize: 12, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.08em",
                  background: mode === m ? G : "transparent",
                  color: mode === m ? "#111" : "rgba(255,255,255,0.4)",
          ...
          <div style={{ marginBottom: 28 }}>
            <h2 style={{ fontSize: 34, fontWeight: 900, color: "#fff", letterSpacing: "-0.04em", lineHeight: 1.1, margin: "0 0 8px" }}>
          ...
          {err && (
            <div style={{
              ...
              background: err.startsWith("✅") ? "rgba(34,197,94,0.1)" : "rgba(239,68,68,0.1)",
              border: `1px solid ${err.startsWith("✅") ? "rgba(34,197,94,0.3)" : "rgba(239,68,68,0.3)"}`,
              color: err.startsWith("✅") ? "#4ade80" : "#f87171",
          ...
          {/* Google button */}
          {mode === "login" && (
            <button onClick={doGoogle} disabled={loading} style={{
              width: "100%", height: 52, background: "rgba(255,255,255,0.05)",
              border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14,
              color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 12,
              marginBottom: 16, fontFamily: "inherit", transition: "background 0.2s",
            }}
            onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.09)"}
            onMouseLeave={e => e.currentTarget.style.background = "rgba(255,255,255,0.05)"}
            >
              <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" style={{ width: 20, height: 20 }} alt="Google" />
              Endelea kwa Google
            </button>
          )}
          ...
          {mode === "login" && (
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em" }}>au</span>
              <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.08)" }} />
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {mode === "register" && (
              <input type="text" placeholder="Jina Kamili" value={name} onChange={e => setName(e.target.value)}
                style={inputStyle}
            ...
            {mode === "register" && (
              <input type="password" placeholder="Thibitisha Password" value={pw2} onChange={e => setPw2(e.target.value)}
                style={inputStyle}
                onFocus={e => e.target.style.borderColor = G}
                onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.1)"}
              />
          ...
          <div style={{ marginTop: 20, textAlign: "center" }}>
            {mode === "login" && (
              <button onClick={() => switchTo("forgot")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.25)", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                Umesahau Password?
              </button>
            )}
            {mode === "forgot" && (
              <button onClick={() => switchTo("login")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                ← Rudi kwenye Login
              </button>
            )}
          </div>
        ...
        {/* RIGHT — Branding panel (hidden on small screens via JS width check) */}
        {typeof window !== "undefined" && window.innerWidth >= 640 && (
          <div style={{
            width: 320, flexShrink: 0, position: "relative", overflow: "hidden",
            background: "linear-gradient(135deg, #0d0f1a 0%, #080a12 100%)",
            ...
            <div style={{
              ...
              background: tog
                ...
                : "linear-gradient(135deg, #F5A623 0%, #FFD17C 100%)",
              transform: tog ? "rotate(0deg) skewY(0deg)" : "rotate(10deg) skewY(40deg)",
            ...
            <div style={{
              ...
              transform: tog ? "rotate(-10deg) skewY(-38deg)" : "rotate(0deg) skewY(0deg)",
              transformOrigin: "bottom left",
              transition: "all 1.4s cubic-bezier(0.4,0,0.2,1)",
              transitionDelay: tog ? "0s" : "0.5s",
            }} />
            <div style={{ position: "relative", zIndex: 2, padding: "48px 40px", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <h3 style={{
                fontSize: 52, fontWeight: 900, lineHeight: 0.9, letterSpacing: "-0.05em",
                color: "#111", margin: "0 0 20px", whiteSpace: "pre-line",
              }}>
                {tog ? "KARIBU\nSTEA" : "KARIBU\nTENA"}
              </h3>
              <p style={{ fontSize: 14, color: "rgba(0,0,0,0.65)", lineHeight: 1.7, maxWidth: 220, margin: "0 0 28px", fontWeight: 500 }}>
                {tog ? "Anza safari yako ya tech. Platform ya kwanza ya tech kwa Watanzania." : "Login uendelee kujifunza na kupata deals bora za kidijitali."}
              </p>
              <div style={{ fontSize: 10, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.12em", color: "rgba(0,0,0,0.35)" }}>
    ...
    <div ref={ref} style={{ position: "relative", flexShrink: 0 }}>
      <button
        ...
        style={{
          ...
          background: hasPhoto ? "transparent" : "linear-gradient(135deg,#F5A623,#FFD17C)",
          cursor: "pointer", padding: 0, overflow: "hidden",
          display: "flex", alignItems: "center", justifyContent: "center",
          flexShrink: 0, transition: "transform 0.15s, box-shadow 0.15s",
          boxShadow: open ? "0 0 0 3px rgba(245,166,35,0.25)" : "none",
        }}
        onMouseEnter={e => e.currentTarget.style.transform = "scale(1.05)"}
        onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
      >
        {hasPhoto
          ? <ProfileImage src={user.photoURL} alt={user.displayName} initial={ini} />
          : <span style={{ fontWeight: 900, fontSize: 15, color: "#111", userSelect: "none" }}>{ini}</span>
        }
      </button>
        ...
        {open && (
          <motion.div
            ...
            style={{
              ...
              width: 240, background: "rgba(11,13,22,0.99)",
              border: "1px solid rgba(255,255,255,0.1)", borderRadius: 18,
              boxShadow: "0 24px 60px rgba(0,0,0,0.6)", padding: 8, zIndex: 1000,
            }}
          >
            {/* Header */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 10px 8px", marginBottom: 6, background: "rgba(255,255,255,0.04)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.05)" }}>
              <div style={{
                width: 36, height: 36, borderRadius: 10, overflow: "hidden", flexShrink: 0,
                background: hasPhoto ? "transparent" : "linear-gradient(135deg,#F5A623,#FFD17C)",
                display: "flex", alignItems: "center", justifyContent: "center",
              }}>
                {hasPhoto ? <ProfileImage src={user.photoURL} alt={user.displayName} initial={ini} /> : <span style={{ fontWeight: 900, fontSize: 13, color: "#111" }}>{ini}</span>}
              </div>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontWeight: 800, fontSize: 13, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 150 }}>{user.displayName || "STEA User"}</div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 150, marginTop: 1 }}>{user.email}</div>
              </div>
            </div>

            {[
              { icon: <User size={14} />, label: "Profile Yangu", fn: () => { onProfile(); setOpen(false); } },
              ...(user.role === "admin" ? [{ icon: <Shield size={14} />, label: "Admin Panel", fn: () => { onAdmin(); setOpen(false); } }] : []),
            ].map((item, i) => (
              ...
              >
                <span style={{ width: 28, height: 28, borderRadius: 8, background: "rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{item.icon}</span>
                {item.label}
              </button>
            ))}
            ...
            <button onClick={() => { onLogout(); setOpen(false); }} style={{
              width: "100%", display: "flex", alignItems: "center", gap: 10,
              padding: "9px 10px", borderRadius: 10, border: "none",
              background: "transparent", color: "rgba(239,68,68,0.75)",
              cursor: "pointer", fontSize: 13, fontWeight: 700, fontFamily: "inherit",
            }}
            ...
            >
              <span style={{ width: 28, height: 28, borderRadius: 8, background: "rgba(239,68,68,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><LogOut size={14} /></span>
              Logout
            </button>
          </motion.div>
        )}
...
function ProfileModal({ user, onClose, onUpdate }) {
  ...
  return createPortal(
    <div
      style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, background: "rgba(4,5,9,0.92)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)" }}
      onClick={e => e.target === e.currentTarget && onClose()}
    >
      <div style={{ position: "relative", width: "100%", maxWidth: 440, background: "#0b0d16", borderRadius: 28, border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 30px 80px rgba(0,0,0,0.7)", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "22px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <h2 style={{ fontSize: 20, fontWeight: 900, color: "#fff", letterSpacing: "-0.03em", margin: 0 }}>Profile Yangu</h2>
          <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.5)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <X size={18} />
          </button>
        </div>
        ...
        <div style={{ padding: 24 }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 28 }}>
            <ProfilePictureUpload userId={user.uid} currentPhotoURL={user.photoURL} onUpdate={url => onUpdate({ ...user, photoURL: url })} />
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label style={{ fontSize: 10, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.15em", color: "#F5A623", display: "block", marginBottom: 8 }}>Jina Kamili</label>
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Weka jina lako..."
                style={{ width: "100%", height: 50, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 600, padding: "0 16px", outline: "none", fontFamily: "inherit" }}
                onFocus={e => e.target.style.borderColor = "#F5A623"}
                onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.1)"}
              />
            </div>
            <div>
              <label style={{ fontSize: 10, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.15em", color: "rgba(255,255,255,0.25)", display: "block", marginBottom: 8 }}>Email</label>
              <div style={{ height: 50, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 12, color: "rgba(255,255,255,0.35)", fontSize: 14, padding: "0 16px", display: "flex", alignItems: "center", overflow: "hidden" }}>
        ...
        <div style={{ padding: "0 24px 24px" }}>
          <button onClick={handleSave} disabled={saving} style={{
            ...
            background: "linear-gradient(135deg,#F5A623,#FFD17C)", color: "#111",
            fontSize: 15, fontWeight: 900, cursor: saving ? "not-allowed" : "pointer",
            opacity: saving ? 0.6 : 1, fontFamily: "inherit", transition: "opacity 0.2s",
          }}>
